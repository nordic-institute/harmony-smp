 ...
 */
package eu.peppol.start.metadata;
 ...
 * incoming in the SOAP header through a SOAPHeaderObject.
 *
...
public class MessageMetadata {
    ...
    /**
     * Default constructor returns a ping message
     */
     public MessageMetadata() {
        //Version Original del PING
        senderValue = "busdox:sender";
        senderScheme = "busdox-actorid-transport";
        recipientValue = "busdox:recipient";
        recipientScheme = "busdox-actorid-transport";
        documentIdScheme = "busdox-docid-qns";
        documentIdValue = "busdox:ping";
        processIdScheme = "busdox-procid-transport";
        processIdValue = "busdox:noprocess";
        messageId = "uuid:" + UUID.randomUUID().toString();
        channelId = "PingInbox";
        SOAPHeaderObject pingHeader = new SOAPHeaderObject();
        pingHeader.setChannelIdentifier(channelId);
        DocumentIdentifierType document = new DocumentIdentifierType();
        document.setScheme(documentIdScheme);
        document.setValue(documentIdValue);
        pingHeader.setDocumentIdentifier(document);
        pingHeader.setMessageIdentifier(messageId);
        ProcessIdentifierType process = new ProcessIdentifierType();
        process.setScheme(processIdScheme);
        process.setValue(processIdValue);
        pingHeader.setProcessIdentifier(process);
        ParticipantIdentifierType recipient = new ParticipantIdentifierType();
        recipient.setScheme(recipientScheme);
        recipient.setValue(recipientValue);
        pingHeader.setRecipientIdentifier(recipient);
        ParticipantIdentifierType sender = new ParticipantIdentifierType();
        sender.setScheme(senderScheme);
        sender.setValue(senderValue);
        pingHeader.setSenderIdentifier(sender);
        this.createDate = new Date();
        this.soapHeader = pingHeader;
    }
