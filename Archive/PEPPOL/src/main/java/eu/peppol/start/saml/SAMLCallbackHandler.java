...
public class SAMLCallbackHandler implements CallbackHandler {
    ...
    private static final Logger logger4J =  Logger.getLogger(SAMLCallbackHandler.class);

    /** Represents the Trustore URL. */
    public static final String KEYSTORE_FILE = "server.keystore";

    /** Represents the Trustore Password. */
    public static final String KEYSTORE_PASSWORD = "server.Keystore.password";

    /** Represents a SAML Issuer. */
    public static final String SAML_TOKEN_ISSUER_NAME = "server.servicename";

    /** Assertion ID prefix. */
    private final String SAML_ID_PREFIX = "SamlID";

    /** Sender ID syntax. */
    private final String SENDER_NAME_ID_SYNTAX =
            "http://busdox.org/profiles/serviceMetadata/1.0/UniversalBusinessIdentifier/1.0/";

    /** Accesspoint ID syntax. */
    private final String ACCESSPOINT_NAME_ID_SYNTAX =
    ...
    private final String AUTHENTICATION_CONTEXT_TYPE =
            "urn:oasis:names:tc:SAML:2.0:ac:classes:X509";

    /** Attribute Assurance Level. */
    private final String ATTRIBUTE_NAME =
            "urn:eu:busdox:attribute:assurance-level";

    /** Attribute Namespace. */
    private final String ATTRIBUTE_NAMESPACE =
            "urn:oasis:names:tc:SAML:2.0:attrname-format:basic";

    /** Authentication Level. */
    private final String AUTHENTICATION_LEVEL = "3";
 ...
 /**
        ...
        for (Callback callback : callbacks) {
            ...
            if (callback instanceof SAMLCallback) {
                ...
                try {
                    ...
                    logger4J.fatal("Error while handling callbacks", ex);
                    java.util.logging.Logger.getLogger(SAMLCallbackHandler.class.getName()).log(Level.SEVERE, ex.getMessage());
                }
     ...
     */
    private Element createSenderVouchesSAMLAssertion(final SAMLCallback samlCallback)
            throws Exception {

        logger4J.debug("Creating and setting the SAML Sender Vouches Assertion.");

        Configuration conf = Configuration.getInstance();

//        Assertion assertion = null;

        String accesspointName = conf.getProperty(SAML_TOKEN_ISSUER_NAME);
        String storeFilename = conf.getProperty(KEYSTORE_FILE);
        String storePassword = conf.getProperty(KEYSTORE_PASSWORD);
        ...
        // Setup SenderNameID
        NameID senderNameID = factory.createNameID(Identifiers.PING_SENDER_VALUE,
                null, SENDER_NAME_ID_SYNTAX);

        // Setup AccessPointID
        NameID accessPointNameID = factory.createNameID(accesspointName, null,
        ...
        AuthnStatement statement = factory.createAuthnStatement(issueInstant,
                        null, ctx, null, null);

        statements.add(statement);
        String assuranceLevel = AUTHENTICATION_LEVEL;
        ...
        Conditions conditions = factory.createConditions(before, after,
                        null, null, null, null);
        Assertion assertion = factory.createAssertion(assertionID,
                        accessPointNameID, issueInstant, conditions,
                        null, subj, statements);
        X509Certificate accessPointCertificate = getCertificate(storeFilename,
                        storePassword);
        Element signedAssertion = sign(assertion, accessPointCertificate,
                        getPrivateKey(storeFilename, storePassword));
