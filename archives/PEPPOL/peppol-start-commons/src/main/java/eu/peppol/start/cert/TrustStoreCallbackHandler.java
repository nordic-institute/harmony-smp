...
public class TrustStoreCallbackHandler implements CallbackHandler {
    ...
    private static final Logger log = Logger.getLogger(TrustStoreCallbackHandler.class);
    ...
    public void handle(final Callback[] callbacks) throws IOException, UnsupportedCallbackException {
        ...
        log.info("handling truststore on: " + TrustStoreCallbackHandler.class.getName());
        Configuration conf = Configuration.getInstance();
        final String truststorePath = conf.getProperty("server.truststore");
        final String truststorePassword = conf.getProperty("server.truststore.password");
        final String truststoreAlias = conf.getProperty("server.truststore.alias");
        ...
        for (final Callback callback : callbacks) {
            ...
            if (callback instanceof KeyStoreCallback) {
                try {
                    ...
                    keyStoreCallBack.setKeystore(getKeystore(truststorePath, truststorePassword));
                } catch (final Exception ex) {
                    ex.printStackTrace();
                }
            } else if (callback instanceof PrivateKeyCallback) {
                try {
                    final KeyStore keystore = getKeystore(truststorePath, truststorePassword);
                    PrivateKey privateKey = null;
                    privateKey = (PrivateKey) keystore.getKey(truststoreAlias, truststorePassword.toCharArray());
