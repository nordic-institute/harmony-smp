...
@Service
public class CRLVerifierServiceImpl extends AbstractServiceImpl implements ICRLVerifierService {
    ...
    public void verifyCertificateCRLs(X509Certificate cert) throws TechnicalException, BusinessException {
        try {
    ...
    private X509CRL downloadCRL(String crlURL) throws IOException,
            ...
            CertificateAuthenticationException, NamingException {
        URL url;
     ...
     */
    public List<String> getCrlDistributionPoints(X509Certificate cert) throws TechnicalException, BusinessException {
    ...
    public void verifyCertificateCRLs(String serial, String crlDistributionPointURL) throws TechnicalException, BusinessException {
        try {
            ...
            if (crl.getRevokedCertificates() != null) {
                for (X509CRLEntry entry : crl.getRevokedCertificates()) {
                    if (new BigInteger(serial.trim().replaceAll("\\s", ""), 16).equals(entry.getSerialNumber())) {
                        throw new CertificateRevokedException("The certificate is revoked by CRL: " + crlDistributionPointURL);
                    }
                }
            } else {
                loggingService.debug("The CRL is null for the given certificate");
            }
        } catch (Exception ex) {
            if (ex instanceof TechnicalException) {
                throw (TechnicalException) ex;
                ...
                throw new CertificateAuthenticationException("Can not verify CRL for certificate of serial number : " + serial, ex);
            }
        }
    }
