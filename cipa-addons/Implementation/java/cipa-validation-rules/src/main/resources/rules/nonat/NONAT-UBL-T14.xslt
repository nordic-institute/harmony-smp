...
<xsl:template match="//cac:CreditNoteLine" mode="M6" priority="1000">
    <svrl:fired-rule context="//cac:CreditNoteLine" />
		...
		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="(cbc:CreditedQuantity and $Prerequisite1) or not ($Prerequisite1)" />
      <xsl:otherwise>
        <svrl:failed-assert test="(cbc:CreditedQuantity and $Prerequisite1) or not ($Prerequisite1)">
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute>
