...
import eu.europa.ec.cipa.webgui.app.components.adapters.InvoiceLineAdapter;
import eu.europa.ec.cipa.webgui.app.components.tables.InvoiceLineTable;
...
public class TabInvoiceLine extends Form {
  ...
  private void initElements () {
    ...
    addBtn.addListener (new Button.ClickListener () {
      ...
      public void buttonClick (final Button.ClickEvent event) {
        ...
        final BIIRULE_T10_R018 biirule_t10_r018 = new BIIRULE_T10_R018 (invoiceLineItem, form);
        ...
        final EUGEN_T10_R018 eugen_t10_r018 = new EUGEN_T10_R018 (form,
                                                                  "Tax Scheme ID",
                                                                  "Tax Category ID",
                                                                  "Tax Total Amount");
        ...
        buttonLayout.addComponent (new Button ("Save invoice line", new Button.ClickListener () {
          ...
          public void buttonClick (final ClickEvent event) {
            final AbstractTextField itemName = (AbstractTextField) form.getField ("Item Name");
            itemName.setMaxLength (50);

            if (itemName.getValue ().toString ().length () > 50) {
              // itemName.setComponentError(new
              // UserError("Item Name should not be more than 50 characters"));
            ...
            try {
              /*
               * Collection<String> props = (Collection<String>)
               * form.getItemPropertyIds(); List<Field> fields = new
               * ArrayList<Field>(); for (String property : props) {
               * fields.add(form.getField(property)); } List<BlurListener>
               * listeners = new ArrayList<BlurListener>(); for (Field f :
               * fields) { if (f instanceof AbstractTextField) {
               * AbstractTextField ff = (AbstractTextField)f;
               * listeners.addAll((Collection<BlurListener>)
               * ff.getListeners(BlurEvent.class)); } }
               * ValidatorsList.validateListenersNotify(listeners);
               * form.validate();
               */
              Utils.validateFormFields (form);
              // form.commit();
              // update table (and consequently add new item to invoiceList
              // list)
    ...
    editBtn.addListener (new Button.ClickListener () {
      ...
      public void buttonClick (final Button.ClickEvent event) {
        ...
        if (rowId != null) {
          ...
          /*
           * // Set invoiceLine 0..N cardinalily panels final Panel
           * itemPropertyPanel = new ItemPropertyForm ("Additional",
           * invoiceLineItem.getInvLineAdditionalItemPropertyList ());
  ...
  /*
   * public Form createGridLayoutInvoiceLineForm() { Form form = new Form() {
   * GridLayout layout = new GridLayout(5,10); Panel pricePanel = new Panel();
   * int counter1 = 0; int counter2 = 0; { layout.setSpacing(true);
   * setLayout(layout); HorizontalLayout h = new HorizontalLayout();
   * h.setWidth("50px"); layout.addComponent(h,2,0); }
   * @Override protected void attachField(Object propertyId, Field field) {
   * //field.setCaption(null); if ("Line Note".equals(propertyId) ||
   ...
   * "Item Description".equals(propertyId) || "Item Name".equals(propertyId) ||
   * "Sellers Item ID".equals(propertyId) ||
   * "Tax Category ID".equals(propertyId) ||
   * "Tax Category Percent".equals(propertyId) ) { Label fieldLabel = new
   * Label(field.getCaption()); //Label fieldLabel =
   * Utils.requiredLabel(field.getCaption()); field.setCaption(null);
   * layout.addComponent(fieldLabel, 0, counter1); layout.addComponent(field, 1,
   * counter1); layout.setComponentAlignment(fieldLabel,
   * Alignment.MIDDLE_RIGHT); counter1++; } else {// Single-column fields Label
   * fieldLabel = new Label(field.getCaption()); field.setCaption(null);
   * layout.addComponent(fieldLabel, 3, counter2); layout.addComponent(field, 4,
   * counter2); layout.setComponentAlignment(fieldLabel,
   * Alignment.MIDDLE_RIGHT); counter2++; } } }; return form; }
   */
    ...
    final Form form = new Form () {
      ...
      protected void attachField (final Object propertyId, final Field field) {
        ...
        if ("Line Note".equals (propertyId) ||
            ...
            "Standard Item ID".equals (propertyId) ||
            "Tax Scheme ID".equals (propertyId) ||
            "Measurement Unit".equals (propertyId)) {

          f1.addComponent (field);
        }
        else
          if ("Price Allowance/Charge Indicator".equals (propertyId) ||
            ...
            if ("Price Allowance/Charge Reason".equals (propertyId))
              ...
              if ("Price Allowance/Charge Multiplier Factor".equals (propertyId))
                field.setCaption ("Multiplier Factor");
              else
                if ("Price Allowance/Charge Amount".equals (propertyId))
                  field.setCaption ("Amount");
                else
                  if ("Price Allowance/Charge Base Amount".equals (propertyId))
  ...
  public Form createInvoiceLineMainForm () {
    // final Form invoiceLineForm = new Form (new FormLayout (), new
    // InvoiceLineFieldFactory ());
    ...
    if (!editMode) {
      ...
      int max = 0;
      for (final InvoiceLineType line : invoiceLineList) {
    ...
    invoiceLineForm.addItemProperty ("Tax Scheme ID", new NestedMethodProperty (invoiceLineItem,
                                                                                "InvLineItemTaxCategoryTaxSchemeID"));
  ...
  class InvoiceLineFieldFactory implements FormFieldFactory {
    ...
    public Field createField (final Item item, final Object propertyId, final Component uiContext) {
      ...
      if ("Measurement Unit".equals (pid)) {
        final UnitCodeSelect unitCodeSelect = new UnitCodeSelect (pid);
            ...
            if ("Line Extension Amount".equals (pid)) {
              ...
              if ("Base Quantity".equals (pid)) {
                ...
                if ("Tax Total Amount".equals (pid)) {
                  ...
                  if ("Invoiced Quantity".equals (pid)) {
                    tf.addValidator (new PositiveValueValidator ());
                  }
                  else
                    if ("Tax Category Percent".equals (pid)) {
                      ...
                      if ("Price Allowance/Charge Multiplier Factor".equals (pid)) {
                        tf.addValidator (new PositiveValueValidator ());
                      }
                      else
                        if ("Price Allowance/Charge Amount".equals (pid)) {
                          tf.addValidator (new PositiveValueValidator ());
                        }
                        else
                          if ("Price Allowance/Charge Base Amount".equals (pid)) {
                            tf.addValidator (new PositiveValueValidator ());
                          }
  ...
  public class BIIRULE_T10_R018 implements ValueChangeListener, ItemSetChangeListener {
    ...
    public BIIRULE_T10_R018 (final InvoiceLineAdapter line, final Form form) {
      this.line = line;
      this.form = form;
    }

    public void calc () {
      final Field lineExtensionAmount = form.getField ("Line Extension Amount");

      final BigDecimal price = line.getInvLinePriceAmount ();
      final BigDecimal baseQuantity = line.getInvLinePriceBaseQuantity ();
      final BigDecimal invoicedQuantity = line.getInvLineInvoicedQuantity ();
      final List <AllowanceChargeType> list = line.getAllowanceCharge ();

      BigDecimal amount = price.divide (baseQuantity).multiply (invoicedQuantity);
      amount.setScale (2, BigDecimal.ROUND_HALF_UP);

      for (final AllowanceChargeType ac : list) {
  ...
  public static class EUGEN_T10_R018 implements ValueChangeListener {
    ...
    public EUGEN_T10_R018 (final Form form,
                           final String taxSchemeField,
                           final String taxCategoryField,
                           final String taxTotalField) {
      this.form = form;
    ...
    @Override
    public void valueChange (final com.vaadin.data.Property.ValueChangeEvent event) {
      final String taxScheme = (String) form.getField (taxSchemeField).getValue ();
      final String taxCategory = (String) form.getField (taxCategoryField).getValue ();
      final Field taxAmount = form.getField (taxTotalField);
