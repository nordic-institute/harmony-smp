...

{
	...
	private Map<String, String> parseInput(ByteArrayInputStream input, HttpServletResponse resp) {
		try {
			...
			MimeMultipart multipart = new MimeMultipart(new ByteArrayDataSource(input, "multipart/signed"));
			...
			for (int i = 0; i < multipart.getCount(); i++) {
				...
				if (part_aux.getContentType() != null && ((part_aux.getContentType().toLowerCase().contains("application/soap+xml"))))
