...
public class ServletVerifyDNS extends HttpServlet {
	...
	private static void _errorLog(@Nonnull final OutputStream aOS, @Nonnull final String sMessage, @Nonnull final Throwable t) throws IOException {
	 ...
	 * @throws IOException
	 */
	private static void _verifyDNSParticipantIdentifier(@Nonnull final OutputStream aOS, @Nonnull final ISMLDNSClient aDNSClient,
			@Nonnull final IGenericDataHandler aGenericHandler, @Nonnull final String sRecordName, @Nonnull final String sReferedName,
			@Nonnull final ParticipantIdentifierType aParticipantID) throws Exception, IOException {
		_infoLog(aOS, " - Participant ID " + aParticipantID.getScheme() + "::" + aParticipantID.getValue() + " with " + sReferedName);
	 ...
	 * @throws IOException
	 */
	private static void _verifyDNSPublisherAnchor(final OutputStream aOS, final ISMLDNSClient aDNSClient, final IGenericDataHandler aGenericHandler,
			final String sReferredName, final String sPublisherAnchorID) throws Throwable {
	...
	private static void _verifyDNSAll(@Nonnull final OutputStream aOS, @Nonnull final ISMLDNSClient aDNSClient,
	 ...
	 * @throws IOException
	 */
	private static void _verifyDBParticipantIdentifier(@Nonnull final OutputStream aOS, @Nonnull final ISMLDNSClient aDNSClient, @Nonnull final String sSMPID,
			@Nonnull final ParticipantIdentifierType aParticipantID) throws Exception, IOException {
		...
		if (sParticipantDNSName == null) {
			...
			if (sDNSHostName == null) {
				...
				// identifier is already in DNS - verify that it points to the
				// correct
				final String sPublisherAnchorID = aDNSClient.getPublisherAnchorFromDnsName(sDNSHostName);
				if (sPublisherAnchorID == null) {
					// IS THIS ERROR ???
					_infoLog(aOS, " -   Could not get PublisherAnchorId from: " + sParticipantDNSName + " : " + sDNSHostName + " : " + sPublisherAnchorID);
				} else {
	...
	private static void _verifyDBAll(final OutputStream aOS, final ISMLDNSClient aDNSClient, final IGenericDataHandler aGenericHandler) throws IOException {
		...
		try {
			...
			for (final String sSMPID : aAllSMPIDs) {
				...
				do {
					final ParticipantIdentifierPageType aParticipantIDPage = aGenericHandler.listParticipantIdentifiers(sNextPageIdentifier, sSMPID);
	...
	public static void verifyAllEntries(@Nonnull final OutputStream aOS) throws Exception {
		...
		try {
			...
			_infoLog(
					aOS,
					"DNSServer is: " + aDNSClient.getServer() + " - handling DNS Zone: " + aDNSClient.getDNSZoneName() + " - SML Zone: "
							+ aDNSClient.getDNSZoneName() + " - TTL: " + DNSClientConfiguration.getTTL() + " secs");
	...
	private static void _deleteAllDNSRecords() throws IOException, ZoneTransferException, IllegalHostnameException {
		...
		for (final Record record : records) {
			...
			} else if (aDnsClient.getPublisherAnchorFromDnsName(name) != null) {
	 ...
	 * @throws IOException
	 */
	private static void _handleRequest(@Nonnull final HttpServletRequest aHttpRequest, @Nonnull final HttpServletResponse aHttpResponse) throws IOException {
	...
	@Override
	protected void doGet(@Nonnull final HttpServletRequest aHttpRequest, @Nonnull final HttpServletResponse aHttpResponse) throws ServletException, IOException {
		_handleRequest(aHttpRequest, aHttpResponse);
	}

	@Override
	protected void doPost(@Nonnull final HttpServletRequest aHttpRequest, @Nonnull final HttpServletResponse aHttpResponse) throws ServletException,
			IOException {
		_handleRequest(aHttpRequest, aHttpResponse);
	}
}
