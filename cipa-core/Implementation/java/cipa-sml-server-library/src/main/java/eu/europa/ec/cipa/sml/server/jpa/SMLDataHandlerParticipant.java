...
public final class SMLDataHandlerParticipant extends JPAEnabledManager implements IParticipantDataHandler {
	...
	private void _internalCreateParticipantIdentifier(@Nonnull final IReadonlyParticipantIdentifier aParticipantIdentifier,
			@Nonnull final DBServiceMetadataPublisher aSMP, @Nonnull final String sClientUniqueID) throws Exception {
		...
		if (aDBIdentifier != null)
			throw new BadRequestException("The participant identifier '" + IdentifierUtils.getIdentifierURIEncoded(aParticipantIdentifier)
					+ "' does already exist.");

		// Find out if this is a Wildcard scheme
		final List<DBAllowedWildcardSchemes> aWildcardSchemes = getEntityManager()
				.createQuery("SELECT p FROM DBAllowedWildcardSchemes p WHERE p.id.scheme=:scheme", DBAllowedWildcardSchemes.class)
	...
	public void deleteParticipantIdentifiers(@Nonnull final List<ParticipantIdentifierType> aParticipantIdentifiers, @Nonnull final String sClientUniqueID)
			throws Throwable {
		JPAExecutionResult<?> ret;
		ret = doInTransaction(new IThrowingRunnable() {
			public void run() throws Exception {
		...
		ret = doInTransaction(new IThrowingRunnable() {
			public void run() throws Exception {
				...
				final DBParticipantIdentifier aDBIdentifier = getEntityManager().find(DBParticipantIdentifier.class,
						new DBParticipantIdentifierID(aParticipantID));
				if (aDBIdentifier == null)
					throw new NotFoundException("The participant identifier '" + IdentifierUtils.getIdentifierURIEncoded(aParticipantID)
	...
	@Nonnull
	public ParticipantIdentifierPageType listParticipantIdentifiers(@Nonnull final PageRequestType aPageRequest, @Nonnull final String sClientUniqueID)
			throws Throwable {
