...
public class ErrorLogger extends AbstractHandler {
    ...
    public InvocationResponse invoke(final MessageContext msgCtx) throws AxisFault {
        ...
        for (final OMElement error : errors) {
            XMLUtil.error(ErrorLogger.log, "Received the following ebms3 Error: ", error);
            ...
            this.emd.persist(errorMessage);

            // Stop sending MSH time schedule due to an unrecoverable error
            this.rtd.updateTrackingStatus(ReceiptTracking.STATUS_UNRECOVERABLE_ERROR, refToMessageInError);
            //TODO: remove attachments
            this.tsd.deleteAttachments(refToMessageInError);
        }
