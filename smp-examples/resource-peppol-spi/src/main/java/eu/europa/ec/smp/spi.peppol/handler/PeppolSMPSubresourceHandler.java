...
import gen.eu.europa.ec.ddc.api.peppol.identifiers.transport.ProcessIdentifier;
...
/**
 * The implementation of the ResourceHandlerSpi for the Peppol SMP ServiceMetadata document.
 *
 * @author Joze Rihtarsic
 * @since 5.2
 */
@Component
public class PeppolSMPSubresourceHandler extends AbstractPeppolSMPHandler {

    private static final Logger LOG = LoggerFactory.getLogger(PeppolSMPSubresourceHandler.class);

    final SmpXmlSignatureApi signatureApi;
    final SmpDataServiceApi smpDataApi;
    final SmpIdentifierServiceApi smpIdentifierApi;
    final SubresourceValidator serviceMetadataValidator;
    final PeppolSMPServiceMetadataReader reader;

    public PeppolSMPSubresourceHandler(SmpDataServiceApi smpDataApi,
                                       SmpIdentifierServiceApi smpIdentifierApi,
                                       SmpXmlSignatureApi signatureApi,
                                       SubresourceValidator serviceMetadataValidator) {
        ...
        this.reader = new PeppolSMPServiceMetadataReader();
    }
    ...
    private static EndpointType getEndpointType() {
        EndpointType endpointType = new EndpointType();
        endpointType.setTransportProfile("bdxr-transport-ebms3-as4-v1p0");
        EndpointReferenceType endpointReferenceType = new EndpointReferenceType();
        AttributedURIType endpointAddress = new AttributedURIType();
        endpointAddress.setValue("https://my.address.eu/endpoint");
        endpointReferenceType.setAddress(endpointAddress);
        endpointType.setEndpointReference(endpointReferenceType);
        endpointType.setCertificate("Certificate data");
