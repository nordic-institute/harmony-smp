 ...
 * #END_LICENSE#
 */
package eu.europa.ec.smp.spi.peppol.validation;
 ...
 */
@Component
public class SubresourceValidator {

    private static final Logger LOG = LoggerFactory.getLogger(SubresourceValidator.class);

    final SmpIdentifierServiceApi smpIdentifierApi;

    public SubresourceValidator(SmpIdentifierServiceApi smpIdentifierApi) {
    ...
    public ServiceInformationType validateServiceInformation(
        ...
        if (Strings.CI.equals(trim(documentIdValue), SUBRESOURCE_IDENTIFIER_VALUE.getPropertyPlaceholder())) {
            documentIdValue = documentIdentifierFromUrl.getValue();
        }

        if (Strings.CI.equals(trim(documentIdScheme), SUBRESOURCE_IDENTIFIER_SCHEME.getPropertyPlaceholder())) {
