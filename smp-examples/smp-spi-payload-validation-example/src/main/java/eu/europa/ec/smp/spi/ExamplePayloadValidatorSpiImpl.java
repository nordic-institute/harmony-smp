    ...
    public void validatePayload(InputStream payload, String mimeType) throws PayloadValidatorSpiException {
        try {
            ...
            if (payload.available() > 0 ) {
                ...
                if (firstChar == (int)'E') {
                    throw new PayloadValidatorSpiException("error.validation.payload.payload.starts.with.e");
                }
            }
        } catch (IOException e) {
            throw new PayloadValidatorSpiException("error.validation.payload.cannot.read.payload", e);
        }
    }
}
